...
Ext.onReady(function(){
    ...
    var svs_id;  
    var  ssn,sname,sid, spec,aty, had, mey,cpu, os,smo,cdt, pono, own, str ,team, apl;
    ...
    var mainForm=new Ext.form.FormPanel(
    {
        ...
        } ,
        {
                    ...
                    handler:function ()
                    {
                        ...
                        ckb_7.setValue(true);
                        ckb_8.setValue(true);
                        txt_server_name.setValue(sname);
                        txt_sn_name.setValue(ssn);
                        txt_ip_address.setValue(sid);
                        txt_server_spec.setValue(spec);
                        cmb_aty_type.setValue(aty);
                        cmb_had_type.setValue(had);   
                        cmb_mey_type.setValue(mey);
                        cmb_cpu_type.setValue(cpu);   
                        cmb_os_type.setValue(os);
                        cmb_smo_type.setValue(smo);  
                        txt_po_no.setValue(pono);
                        Ext.getCmp('txt_ap_list').setValue(apl);
                        txt_owner.setValue(own);
                        txt_come_dt.setValue(cdt);
                        cmb_str_type.setValue(str);
                        cmb_team_type.setValue(team);
                    }
   ...
   function comfirmFun()
   {
    ...
    lst.push(AddEntryJson);
       if(mainForm.form.isValid())
       {
                ...
                url:'../../ashx/Server/ModifyServerList.ashx',
                method:'post',
                params:
                {
...
{ 
    Ext.Ajax.request(
    {
        url: '../../ashx/Server/GetServerListForModify.ashx',    
        method: 'POST',
        disableCaching:true,
        params:
        {
           SVS_ID:GetQueryString('SVS_ID')
        },
        success:function (result,request)
        {
            
            var json_data=Ext.util.JSON.decode(result.responseText);
            svs_id=json_data.SVS_ID;
            txt_server_name.setValue(json_data.SVS_NAME);
            txt_sn_name.setValue(json_data.SVS_SN);
            txt_ip_address.setValue(json_data.SVS_IPADDRESS);
            txt_server_spec.setValue(json_data.SVS_SHORTDESC);
            cmb_aty_type.setValue(json_data.ATY_ID);
            cmb_had_type.setValue(json_data.HAD_ID);   
            cmb_mey_type.setValue(json_data.MEY_ID);
            cmb_cpu_type.setValue(json_data.CPU_ID);   
            cmb_os_type.setValue(json_data.OS_ID);
            cmb_smo_type.setValue(json_data.SMO_ID);   
            txt_come_dt.setValue(json_data.SER_COME_DT);
            txt_po_no.setValue(json_data.SER_PO_NO);
            txt_owner.setValue(json_data.SER_OWNER);
            cmb_str_type.setValue(json_data.SER_STR_ID);
            cmb_team_type.setValue(json_data.SER_TEP_ID);
            Ext.getCmp('txt_ap_list').setValue(json_data.SER_AP_LIST);  
                      
             ssn=json_data.SVS_SN;
             sname=json_data.SVS_NAME;
             sid=json_data.SVS_IPADDRESS;
             spec=json_data.SVS_SHORTDESC;
             aty=json_data.ATY_ID;
             had=json_data.HAD_ID;
             mey=json_data.MEY_ID;
             cpu=json_data.CPU_ID;
             os=json_data.OS_ID;
             smo=json_data.SMO_ID;
            
             cdt=json_data.SER_COME_DT;
             pono=json_data.SER_PO_NO;
             own=json_data.SER_OWNER;
             str=json_data.SER_STR_ID;
             team=json_data.SER_TEP_ID;
             apl=json_data.SER_AP_LIST;
        },
