    ...
    public partial class newsajax : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(Request["page"]))
            {
                ...
                string sqlstr = "select top " + Pagesize + " * from Sys_News where iID not in (select top " + PageCount + " iID from Sys_News where (iTypeID=7 or iTypeID=9 or iTypeID=10) and bIsAudit = 1 order by iID desc) and (iTypeID=7 or iTypeID=9 or iTypeID=10) and bIsAudit = 1 order by iID desc";
