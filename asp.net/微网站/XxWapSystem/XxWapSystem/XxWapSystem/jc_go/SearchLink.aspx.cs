    ...
    public partial class SearchLink : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                strWhere = strWhere + " and cProductName like '%" + type + "%'";
                string sql = "select * from Sys_Product where " + strWhere + " order by iID desc";
                ...
                if (dt.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Tables[0].Rows.Count; i++)
                    {
                        ...
                        {
                            sb.Append("{\"id\":\"" + dt.Tables[0].Rows[i]["iID"].ToString() + "\",\"name\":\"" + dt.Tables[0].Rows[i]["cProductName"].ToString() + "\"}");
                        }
                        else
                        {
                            sb.Append("{\"id\":\"" + dt.Tables[0].Rows[i]["iID"].ToString() + "\",\"name\":\"" + dt.Tables[0].Rows[i]["cProductName"].ToString() + "\"},");
