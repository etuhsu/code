...
var BaseList=new Ext.form.FieldSet(
    {
        ...
        items:[{
                    ...
                    [{
                       layout:'form',
                       columnWidth:.5,
                       items: [txt_application,txt_IT_cost,txt_user_benefit]
                    },
                    {
                          layout:'form',
                          columnWidth:.5,
...
function GetData()
{
    Ext.Ajax.request(
    {
        url: '../../ashx/SR/GetSRListBySR_IDForModify.ashx',
        method: 'POST',
        params:
        {
           SR_ID:GetQueryString('SR_ID')
        ...
        success:function (result,request)
        {
            ...
            var  json_data=Ext.util.JSON.decode(result.responseText).LIST;

                txt_IT_owner.setValue(json_data[0].SR_IT_OWNER);
                txt_sod_expect_dt.setValue(json_data[0].SR_SOD_EXPECT);
                txt_sod_update.setValue(json_data[0].SR_SOD_UPDATE);
                txt_sr_title.setValue(json_data[0].SR_TITLE);


                cmb_sr_sort.setValue(json_data[0].RAT_ID);
                cmb_sr_group.setValue(json_data[0].SR_GRP_ID);
                cmb_sr_level.setValue(json_data[0].LEL_ID);
                txt_application.setValue(json_data[0].SR_PROPOSER);
                txt_IT_cost.setValue(json_data[0].SR_IT_COST);
                txt_user_benefit.setValue(json_data[0].SR_USER_BENEFIT);
                txt_sr_client.setValue(json_data[0].SR_CLIENT);
                cmb_apply.setValue(json_data[0].DET_ID);
                cmb_sys_type.setValue(json_data[0].SYE_ID);
                txt_over_dt.setValue(json_data[0].SR_OVERDATE);
                
                Ext.getCmp('txt_requirement').setValue(json_data[0].SR_DEMAND_DESC);
                if(json_data[0].SR_PRO_FLAG==true)
                {
                   chk_project.setValue(true);
                }
                else  if(json_data[0].SR_SR_FLAG==true)
                {
                    chk_sr.setValue(true);
                }
                else if(json_data[0].SR_AR_FLAG==true)
                {
                    chk_ar.setValue(true);
                }
        },
        failure: function()
        {
