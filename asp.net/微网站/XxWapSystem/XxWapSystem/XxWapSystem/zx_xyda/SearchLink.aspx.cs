    ...
    public partial class SearchLink : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                strWhere = strWhere + " and cqyname like '%" + type + "%'";
                string sql = "select * from xyda_jb where " + strWhere + " order by iID desc";
                ...
                if (dt.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Tables[0].Rows.Count; i++)
                    {
                        ...
                        {
                            sb.Append("{\"id\":\"" + dt.Tables[0].Rows[i]["iID"].ToString() + "\",\"name\":\"" + dt.Tables[0].Rows[i]["cqyname"].ToString() + "\"}");
                        }
                        else
                        {
                            sb.Append("{\"id\":\"" + dt.Tables[0].Rows[i]["iID"].ToString() + "\",\"name\":\"" + dt.Tables[0].Rows[i]["cqyname"].ToString() + "\"},");
