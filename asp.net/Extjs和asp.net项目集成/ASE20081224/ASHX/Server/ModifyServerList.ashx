<%@ WebHandler Language="C#" Class="ModifyServerList" %>
using System;
using System.Web;
...
public class ModifyServerList : IHttpHandler, IRequiresSessionState
{
      ...
      /*  if (string.IsNullOrEmpty(context.Request["txt_server_name"]) == true)
        ...
        using (SqlConnection conn = new SqlConnection(SqlHelper.ConnectionString))
        {
            ...
            try
            {
                ...
                model.SVS_ID = WebUtility.InputInt(context.Request["SVS_ID"]);
                model.SVS_SN = WebUtility.InputText(context.Request["txt_sn_name"].ToString(), 50);
                model.SVS_UPDATEUID = userStr;
                ...
                model2.SER_OPERATE_DT = DateTime.Now;
                model2.SER_SVS_ID=WebUtility.InputInt(context.Request["SVS_ID"]);
                if (bll.ModifyRServerListBySVS_ID(trans,model2) > 0 && bll.ModifyServerListBySVS_ID(trans,model) > 0)
                {
                    trans.Commit();
