...
function initImages(oid) { 
    var fmt = "<li {$co$}><a   href=other/.cn/qizhongdianyongcitie/{$path$} target=\"_blank\"><img longdesc=other/.cn/qizhongdianyongcitie/{$path$} src=other/.cn/qizhongdianyongcitie/{$path$}  width=\"60\" height=\"60\"></a></li>";
    var oHtml = "";
...
//function initImages(oid) {
//    var fmt = "<a href=\"other/.cn/qizhongdianyongcitie/{$path$}\" rel=\"zoom1\" rev=\"other/.cn/qizhongdianyongcitie/{$path$}\" title=\"\">"
//        + "<img src=\"/tools/photo.aspx?p=other/.cn/qizhongdianyongcitie/{$path$}&t=0&w=60&h=60\"  class=\"blackborder\" rel=\"zoom1\"  rel=\"thumb-change: mouseover\" rev=\"/tools/photo.aspx?p=other/.cn/qizhongdianyongcitie/{$path$}&t=0&w=60&h=60\" /></a>";
//    var oHtml = "";
//    for (var i = 0; i < ARR_IMG_PATH.length; ++i) {
//        oHtml += fmt.replace(/\{\$path\$\}/ig, ARR_IMG_PATH[i]);
//    }
...
//function HelpMenuSelectPosition(MARK, current, sid) {
...
//        muneCur();
//    } else {
//        $("#menulist a[href='/sibeikejianjie.html']").parent("li").addClass("dq").siblings("li").removeClass("dq");
//    }
//}
/*********** 帮助栏目导航定位
*导航是否有帮助中心
******/
function HelpMenuSelectPosition(iscolumn) {
    var currenturl = "sibeikejianjie.html";
    var info = "div.plc a";
    var count = $(info).size();
    for (var i = count - 1; i >= 0; i++) {
        ...
        if (current == "" || current == undefined) {
            current = iscolumn ? "help" : currenturl;
        }
        var a = $("#menulist a[href*='" + current + "']").size();
        if ($("#menulist a[href*='" + current + "']").size()) {
            $("#menulist a[href*='" + current + "']").parent("li").addClass("dq").siblings("li").removeClass("dq");
            return false;
        } else {
            $("#menulist a[href*='" + currenturl + "']").parent("li").addClass("dq").siblings("li").removeClass("dq");
        }
    }
    if (!$("#menulist li[class='dq']").size()) {
        $("#menulist li:first").addClass("dq").siblings("li").removeClass("dq");
    }
}
/*********** 新闻栏目导航定位
*导航是否有产品中心
******/
function ProductMenuSelectPosition(iscolumn) {//mark, current, sid
    var currenturl = "";
    var count = $("div.pro_curmbs a").size();
    for (var i = count - 1; i >= 0; i--) {
        var current = $("div.pro_curmbs a").eq(i).attr("href");
        current = current.substring(current.lastIndexOf("/") + 1);
        if (current == "" || current == undefined) {
            current = iscolumn ? "product" : currenturl;
        }
        var a = $("#menulist a[href*='" + current + "']").size();
        if ($("#menulist a[href*='" + current + "']").size()) {
            $("#menulist a[href*='" + current + "']").parent("li").addClass("dq").siblings("li").removeClass("dq");
    ...
    if (!$("#menulist li[class='dq']").size()) {
        $("#menulist li a[href*='product']").parent("li").addClass("dq").siblings("li").removeClass("dq");
        if (!$("#menulist li[class='dq']").size()) {
            $("#menulist li:first").addClass("dq").siblings("li").removeClass("dq");
        }
    }
}
function ProjectMenuSelectPosition(mark, current, sid) {

}
/*********** 新闻栏目导航定位
*是否动态页面态页面 1 是动态页面 0是静态页面
*导航是否有新闻中心
******/
function NewsMenuSelectPosition(current, iscolumn) {//mark,
    //如果导航没有新闻中心的话，就找一个样式停留的跟资讯相关的链接
    var currenturl = "gongsidongtai.shtml";
    //是否是静态页面，静态页面的面包屑导航
    var info = current ? "div.plc a" : "div.plc2 a";
    //获取面包屑导航的链接的个数
    var count = $(info).size();
    //倒序查找链接地址
    for (var i = count - 1; i >= 0; i--) {
        //获取url 链接
        var current = $(info).eq(i).attr("href");
        var txt = $(info).eq(i).text();
        //获取要匹配的导航链接
        current = current.substring(current.lastIndexOf("/") + 1);
        if (current == "" || current == undefined) {
            current = iscolumn ? "news" : currenturl;
        }
        var a = $("#menulist a[href*='" + current + "']").size();
        /*
        *
        */
        if ($("#menulist a:contains('" + txt + "')").size() && txt != "首页") {
            $("#menulist a:contains('" + txt + "')").parent("li").addClass("dq").siblings("li").removeClass("dq");
            return;
        }else
        /*
        *
        */
        //判断导航链接是否有面包屑导航相同匹配的个数
        if ($("#menulist a[href*='" + current + "']").size()) {
            $("#menulist a[href*='" + current + "']").parent("li").addClass("dq").siblings("li").removeClass("dq");
            return;
        }
...
*导航是否有下载中心
****************/
function DownloadMenuSelectPosition(iscolumn) {
    var currenturl = "";
    var info = "div.plc a";
    var count = $(info).size();
    for (var i = count - 1; i >= 0; i--) {
        ...
        current = current.substring(current.lastIndexOf("/") + 1);
        if (current == "" || current == undefined) {
            current = iscolumn ? "download" : currenturl;
        }
        var a = $("#menulist a[href*='" + current + "'").size();
        if ($("#menulist a[href*='" + current + "']").size()) {
            $("#menulist a[href*='" + current + "']").parent("li").addClass("dq").siblings("li").removeClass("dq");
