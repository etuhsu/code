...
{
    public partial class newsajax_jj : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(Request["page"]))
            {
                ...
                string sqlstr = "select top " + Pagesize + " * from Sys_News where iID not in (select top " + PageCount + " iID from Sys_News where (iTypeID=3 or iTypeID=2 or iTypeID=26) and bIsAudit = 1 order by iID desc) and (iTypeID=3 or iTypeID=2 or iTypeID=26) and bIsAudit = 1 order by iID desc";
